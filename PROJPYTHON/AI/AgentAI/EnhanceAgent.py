...
from AI.AgentAI.Utils.AllClass import Enhance,MessageState
class EnhanceAi:
    def __init__(self):
    ...
    async def final_Answer(self,state:MessageState):
        sys_mdg=SystemMessage(content=self.prompt.format(query=state["messages"][-1].content))
    ...
    async def answer(self,q):
        ...
        input_={"messages":[HumanMessage(content=q)]}
